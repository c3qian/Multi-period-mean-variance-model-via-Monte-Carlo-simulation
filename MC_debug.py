...
for i in range(20):
    ...
    stdFwdList_nobkcy =[]
    muFwdList_nobkcy = []
    returns,increments = simulateReturnPathsGBM(r, sigma, xi,dt, numOfPath,timesteps)
    gammaList = np.linspace(9.125,85.125, 80)
    for gamma in gammaList:
        termWealth, termWealth_nobkcy,termWealth_bdd,optPs,optP_bdds,optP_nobkcys = multiStageStrategyVectorize(gamma,returns,increments,S0,r,xi,sigma,C,numOfPath,M,T,dt,W0,Pmax,Pmin,alpha,timesteps)
...
def backwardStrategyTest(gamma,Type,fromMultiPeriod,RETURNS,INCRE):
    """
    PARAMETERS: 
...
# %%
print(f"the terminal wealth is given by {termWealth}")
print(f"the nobkcy terminal wealth is given by {termWealth_nobkcy}")
print(f"the bdd terminal wealth is given by {termWealth_bdd}")

print(f"the return is given by {optPs}")
print(f"the nobkcy return is given by {optP_nobkcys}")
print(f"the bdd return is given by {optP_bdds}")
